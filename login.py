...
class Login:
    def __init__(self, root):
        ...
        self.title_frame = Frame(
            self.root, bd=4, relief=RIDGE, bg="white")
        self.title_frame.place(x=0, y=0, width=560, height=326)

        # Title Head
        self.lbl_title = Label(
            self.title_frame, text="Login Form", bd=4, relief=RIDGE,  bg="black", fg="#fc5c00", font=("roboto sans-serif", 23), pady=14)
        ...
        self.fname_label = Label(self.form_frame, text='',
                                 font=('bold', 24), bg="white", fg="black", padx=20, pady=7).grid(row=2, column=2, sticky=W)
        # Username
        self.username_text = StringVar()
        self.username_label = Label(self.form_frame, text='Username :',
                                    font=("roboto sans-serif", 14), bg="white", fg="black", padx=5, pady=10).grid(
        ...
        self.password_label = Label(self.form_frame, text='Password :',
                                    font=("roboto sans-serif", 14), bg="white", fg="black", padx=5, pady=14).grid(
        ...
        self.back_btn = Button(self.btn_frame, text='Register Now', width=15, height=2,
                               bg="#fc5c01", fg="black", bd=2, relief=FLAT, command=self.register, font=("roboto sans-serif", 10, "bold"))
    ...
    # Accessing the table in the database to check
    def fields(self):
        ...
        if self.username_entry.get() == '' or self.password_entry.get() == '':
            ...
            try:
                con = pymysql.connect(
                    host="localhost", database2="root", password="hello1234", database="register")
                cur = con.cursor()
                ...
                if row == None:
                    ...
                    messagebox.showinfo(
                        'Success', 'Welcome to Courier Management Database')
                    self.clear()
                    self.root.destroy()
                    import dashboard
                con.close()
    ...
    # to enter the register page

    def register(self):

        top = Toplevel()
        top.title("Registration Form for Employees only")
        ...
        def register_data():
            ...
            # check the password. Add a layer of security
                ...
                try:
                    con = pymysql.connect( #Connection to the mysql
                        host="localhost", user="root", password="hello1234", database="register")
                    cur = con.cursor()
                    cur.execute("select * from user where email=%s",
                                email_text.get())
                    row = cur.fetchone()

                    # checking whether the user has registred or not before
                    if row != None:
                        messagebox.showerror(
                            'Error', 'Your Email is Already Exist ')

                    # Values are uploaded to the database 
                    else:
                        cur.execute(
        ...
        # Title Head
        top.lbl_title = Label(
            top, text="Registration Form for employees only", bd=4, fg="black", relief=RIDGE,  bg="white", font=("roboto sans-serif", 23), pady=7)
        top.lbl_title.pack(side=TOP, fill=X)

        # Frames
        top.title_frame = Frame(
            top, bd=4, relief=RIDGE, bg="white")
        top.title_frame.place(x=0, y=55, width=530, height=417)

        #Space at the center
        top.fname_label = Label(top.title_frame, text='',
                                font=('bold', 14), bg="white", fg="black", padx=12, pady=20).grid(
            row=2, column=2, sticky=W)

        # Firstname
        fname_text = StringVar()
        top.fname_label = Label(top.title_frame, text='First Name :',
                                font=("roboto sans-serif", 14), bg="white", fg="black", padx=10, pady=20).grid(
        ...
        top.lname_label = Label(top.title_frame, text='Last Name :',
                                font=("roboto sans-serif", 14), bg="white", fg="black", padx=10).grid(
            row=3, column=3, sticky=W)
